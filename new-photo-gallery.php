...
if ( ! class_exists( 'New_Light_Image_Gallery' ) ) {
	...
	class New_Light_Image_Gallery {
		...
		protected function _hooks() {
			...
			//add gallery menu item, change menu filter for multisite
			add_action( 'admin_menu', array( $this, '_srgallery_featured_plugins_menu' ), 105 );
		...
		public function _srgallery_menu() {
			...
			$upgrade_plugin_menu = add_submenu_page( 'edit.php?post_type='.LG_PLUGIN_SLUG, __( 'Upgrade Plugin', 'LGP_TXTDM' ), __( 'Upgrade Plugin', 'LGP_TXTDM' ), 'administrator', 'sr-upgrade-pg-plugin-page', array( $this, '_upgrade_pg_page') );
		}		
		
		public function _srgallery_featured_plugins_menu() {
			$help_menu = add_submenu_page( 'edit.php?post_type='.LG_PLUGIN_SLUG, __( 'Featured Plugins', 'LGP_TXTDM' ), __( 'Featured Plugins', 'LGP_TXTDM' ), 'administrator', 'sr-featured-plugins-page', array( $this, '_lg_featured_plugins') );
		}		
		...
		public function _lg_featured_plugins() {
			require_once('featured-plugins/featured-plugins.php');
		}	
		
		public function _upgrade_pg_page() {
			require_once('buy-photo-gallery-premium.php');
		}		
	}// end of class
